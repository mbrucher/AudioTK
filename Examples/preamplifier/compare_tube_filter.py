...
def compute_and_plot(d, txt):
  ...
  out_guitar = filter_4(d, Rp=220e3, Rg=68e3, Ro=1e6, Rk=1.5e3, VBias=300, Co=47e-9, Ck=22e-6)
...
if __name__ == "__main__":
  ...
  d = np.sin(x * 2 * np.pi * 40) * 10
  compute_and_plot(d, "30")
  d = np.sin(x * 2 * np.pi * 200) * 10
  compute_and_plot(d, "200")
  d = np.sin(x * 2 * np.pi * 1000) * 10
  compute_and_plot(d, "1k")
  d = np.sin(x * 2 * np.pi * 10000) * 10
  compute_and_plot(d, "10k")
