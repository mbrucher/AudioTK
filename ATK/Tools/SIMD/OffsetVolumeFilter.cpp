...
namespace ATK
{
  ...
  namespace SIMDPP_ARCH_NAMESPACE
  {
    ...
    std::unique_ptr<BaseFilter> createOffsetVolumeFilter(gsl::index nb_channels)
    {
  ...
  SIMDPP_MAKE_DISPATCHER((template<typename DataType, gsl::index VL>) (<DataType, VL>) (std::unique_ptr<BaseFilter>) (createOffsetVolumeFilter)
                         ((gsl::index) nb_channels))

  SIMDPP_INSTANTIATE_DISPATCHER(
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createOffsetVolumeFilter<double, 2>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createOffsetVolumeFilter<float, 4>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createOffsetVolumeFilter<double, 4>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createOffsetVolumeFilter<float, 8>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createOffsetVolumeFilter<double, 8>(gsl::index)));
}
