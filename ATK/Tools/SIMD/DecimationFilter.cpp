...
namespace ATK
{
  ...
  namespace SIMDPP_ARCH_NAMESPACE
  {
    ...
    std::unique_ptr<BaseFilter> createDecimationFilter(std::size_t nb_channels)
    {
      return std::unique_ptr<BaseFilter>(new DecimationFilter<typename SIMDTypeTraits<DataType>::template SIMDType<VL> >(nb_channels));
    }
  }
  
  SIMDPP_MAKE_DISPATCHER((template<typename DataType, std::size_t VL>) (<DataType, VL>) (std::unique_ptr<BaseFilter>) (createDecimationFilter)
                         ((std::size_t) nb_channels))

  SIMDPP_INSTANTIATE_DISPATCHER(
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createDecimationFilter<double, 2>(std::size_t)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createDecimationFilter<float, 4>(std::size_t)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createDecimationFilter<double, 4>(std::size_t)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createDecimationFilter<float, 8>(std::size_t)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createDecimationFilter<double, 8>(std::size_t)));
}
