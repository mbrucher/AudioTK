...
namespace ATK
{
  ...
  namespace SIMDPP_ARCH_NAMESPACE
  {
    ...
    std::unique_ptr<BaseFilter> createVolumeFilter(gsl::index nb_channels)
    {
      return std::unique_ptr<BaseFilter>(new VolumeFilter<typename SIMDTypeTraits<DataType>::template SIMDType<VL> >(nb_channels));
    }
  }
  
  SIMDPP_MAKE_DISPATCHER((template<typename DataType, gsl::index VL>) (<DataType, VL>) (std::unique_ptr<BaseFilter>) (createVolumeFilter)
                         ((gsl::index) nb_channels))

  SIMDPP_INSTANTIATE_DISPATCHER(
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createVolumeFilter<double, 2>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createVolumeFilter<float, 4>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createVolumeFilter<double, 4>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createVolumeFilter<float, 8>(gsl::index)),
    (template ATK_TOOLS_EXPORT std::unique_ptr<BaseFilter> createVolumeFilter<double, 8>(gsl::index)));
}
