...
from ATK.Delay import DoubleQuadHadamardFeedbackDelayNetworkFilter
...
def hadamard_test():
  ...
  d = np.fromfile(dirname + os.sep + "input_hadamard.dat", dtype=np.float64).reshape(1, -1)
  ref = np.fromfile(dirname + os.sep + "output_hadamard.dat", dtype=np.float64).reshape(1, -1)
