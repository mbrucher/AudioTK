...
from ATK.EQ import DoubleChebyshev2LowPassFilter, DoubleChebyshev2HighPassFilter, DoubleChebyshev2BandPassFilter, DoubleChebyshev2BandStopFilter
...
def filter_low(input):
  ...
  lowpassfilter = DoubleChebyshev2LowPassFilter()
...
def filter_band(input):
  ...
  infilter = DoubleInPointerFilter(input, False)
  infilter.input_sampling_rate = 48000
  bandpassfilter = DoubleChebyshev2BandPassFilter()
...
def filter_bandstop(input):
  ...
  infilter = DoubleInPointerFilter(input, False)
  infilter.input_sampling_rate = 48000
  bandstopfilter = DoubleChebyshev2BandStopFilter()
...
def cheby2_lowpass_test():
  ...
  bref, aref = cheby2(5, 3, 1000/24000.)
...
def cheby2_bandpass_test():
  ...
  bref, aref = cheby2(5, 3, (200/24000., 1000/24000.), btype='bandpass')
  
  assert_almost_equal(-b, bref)
  assert_almost_equal(np.hstack(([1], -a[::-1])), aref)

def cheby2_bandstop_test():
  import numpy as np
  from numpy.testing import assert_almost_equal
  ...
  bref, aref = cheby2(5, 3, (200/24000., 1000/24000.), btype='bandstop')
