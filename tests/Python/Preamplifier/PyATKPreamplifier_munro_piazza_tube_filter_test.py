...
from ATK.Preamplifier import DoubleMunroPiazzaTriodeFilter
...
def filter_32(input):
  ...
  overdrivefilter = DoubleMunroPiazzaTriodeFilter.build_standard_filter()
...
def modified_munro_piazza_32_test():
  ...
  ref = np.fromfile(dirname + os.sep + "output_tube_32mp.dat", dtype=np.float64).reshape(1, -1)
  out = filter_32(x)
...
if __name__ == "__main__":
  ...
  out = filter_32(d)
  out.tofile("output_tube_32mp.dat")
  out = filter_16(d)
  out.tofile("output_tube_16mp.dat")
  out = filter_8(d)
  out.tofile("output_tube_8mp.dat")
  out = filter_4(d)
  out.tofile("output_tube_4mp.dat")
